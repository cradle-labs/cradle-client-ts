...
type CradleNativeListingFunctionsInput =
  ...
  | {
      CreateListing: {
        ...
        documents: string;
        company: UUID;
        asset: { Existing: UUID } | { New: { asset_type: AssetType; name: string; symbol: string; decimals: number; icon: string } };
        purchase_asset: UUID;
        purchase_price: Big;
        max_supply: Big;
      };
...
class CradleClient {
  ...
  getBalance(walletId: UUID, assetId: UUID): Promise<ApiResponse<{ balance: number; before_deductions: number; deductions: number; decimals: number }>> {
  ...
  // Orders
  listOrders(filters?: { wallet?: UUID; market_id?: UUID; status?: OrderStatus; order_type?: OrderType; mode?: FillMode }): Promise<ApiResponse<OrderBookRecord[]>> {
  ...
  // Time series
  getTimeSeriesHistory(params: { market: UUID; duration_secs: Big | number; interval: TimeSeriesInterval; asset_id: UUID }): Promise<ApiResponse<MarketTimeSeriesRecord[]>> {
  ...
  // Listings
  listListings(filters?: { company?: UUID; listed_asset?: UUID; purchase_asset?: UUID; status?: ListingStatus }): Promise<ApiResponse<CradleNativeListingRow[]>> {
    const query = this.queryString(filters);
  ...
  getDepositPosition(poolId: UUID, walletId: UUID): Promise<ApiResponse<GetUserDepositPositonOutput>> {
  ...
  private async request<T>(path: string, config: AxiosRequestConfig, opts?: { auth?: boolean }): Promise<T> {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(status ? `Request failed (${status}): ${message}` : `Request failed: ${message}`);
      }
